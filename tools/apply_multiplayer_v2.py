...
if 'belote_multiplayer.js' not in html:
    ...
    old_bot = "function nextBot(){if(!state||state.phase==='done'||canHuman(state.active)||state.multiplayer)return;let token=++state.botTimer,delay=state.phase==='bid'?(state.diff==='calm'?1350:1050):(state.diff==='calm'?950:680);await sleep(delay);if(!state||token!==state.botTimer||canHuman(state.active)||state.multiplayer)return;let p=state.active;if(state.phase==='bid'){let s=botSuit(p);bid(p,s||'pass')}else if(state.phase==='play'){let l=legal(p);let choice=l.sort((a,b)=>points(a,state.trump)-points(b,state.trump)||strength(a,state.trump)-strength(b,state.trump))[0];play(p,choice.id)}}"
