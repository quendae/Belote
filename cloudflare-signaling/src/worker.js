...
export default {
  async fetch(request, env) {
    ...
    if (url.pathname === "/api/health" && request.method === "GET") {
      return json({ ok: true, service: "belote-signaling" });
    }
...
export class SignalingRoom extends DurableObject {
  ...
  async authenticate(socket, message) {
    const meta = await this.ctx.storage.get("meta");
    if (!meta || meta.expiresAt <= Date.now()) return this.closeSocket(socket, 4004, "Room expired");

    if (message.role === "host") {
      if (message.token !== meta.hostToken) return this.closeSocket(socket, 4003, "Invalid host token");
    ...
    if (message.role !== "guest" || String(message.auth || "").toLowerCase() !== meta.auth || !validNick(message.nick)) {
      return this.closeSocket(socket, 4003, "Invalid room, password or nickname");
    }

    const guests = (await this.ctx.storage.get("guests")) || [];
    if (guests.length >= 3) return this.closeSocket(socket, 4009, "Room full");
  ...
  async webSocketMessage(socket, rawMessage) {
    ...
    if (text.length > MAX_SIGNAL_MESSAGE_BYTES) return this.closeSocket(socket, 4009, "Message too large");

    let message;
    try { message = JSON.parse(text); } catch { return this.closeSocket(socket, 4002, "Invalid message"); }

    const attachment = socket.deserializeAttachment() || { role: "pending" };
    if (attachment.role === "pending") {
      if (message.type !== "authenticate") return this.closeSocket(socket, 4003, "Authentication required");
      await this.authenticate(socket, message);
      return;
    }
    if (attachment.role === "guest") return this.handleGuestMessage(socket, attachment, message);
  ...
  async handleGuestMessage(socket, attachment, message) {
    ...
    if (index < 0) return this.closeSocket(socket, 4004, "Guest session expired");

    if (message.type === "offer") {
      if (!validSessionDescription(message.sdp, "offer")) return this.closeSocket(socket, 4002, "Invalid WebRTC offer");
  ...
  async handleHostMessage(message) {
    if (message.type === "answer") {
      if (!validSessionDescription(message.sdp, "answer") || ![1, 2, 3].includes(message.seat)) return;
      const guests = (await this.ctx.storage.get("guests")) || [];
    ...
    if (message.type === "reject") {
      let guests = (await this.ctx.storage.get("guests")) || [];
      const guest = guests.find((item) => item.id === message.guestId);
      if (!guest) return;
      this.sendGuest(message.guestId, { type: "rejected", reason: String(message.reason || "room_full") });
      for (const socket of this.sockets("guest", message.guestId)) this.closeSocket(socket, 4009, "Rejected by host");
      guests = guests.filter((item) => item.id !== message.guestId);
      await this.ctx.storage.put("guests", guests);
      return;
    }
